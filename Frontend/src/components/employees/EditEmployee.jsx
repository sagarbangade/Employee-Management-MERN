...
import { getEmployeeDetails, updateEmployee, clearError } from '../../store/actions/employeeActions';
...
const EditEmployee = () => {
    ...
    if (loading && !employee) { // Initial loading
        return <Box textAlign="center" mt={8}><Spinner size="xl" /></Box>;
    }

    if (!employee) { // Employee details not loaded yet or not found
        return <Box textAlign="center" mt={8}>Employee not found.</Box>;
    }

    return (
        <Box maxW="md" mx="auto" mt={8} p={6} shadow="md" borderWidth="1px" borderRadius="md">
            <Heading textAlign="center" mb={4}>Edit Employee</Heading>
            {error && (
                        ...
                        <Form>
                            <VStack spacing={4} align="stretch">
                                ...
                                {employee.profilePictureUrl && (
                                    <Box mb={3}>
                                        <Image src={employee.profilePictureUrl} alt="Current Profile" boxSize="100px" borderRadius="md" />
                                        <FormHelperText>Current Profile Picture</FormHelperText>
                                    </Box>
                                )}

                                <FormControl>
                                    <FormLabel htmlFor="profilePicture">Update Profile Picture (Optional)</FormLabel>
                                    <Input type="file" id="profilePicture" name="profilePicture" onChange={handleFileChange} accept="image/*" />
                                    <FormHelperText>Upload new employee profile picture to replace the existing one (JPEG, PNG, JPG)</FormHelperText>
                                </FormControl>
